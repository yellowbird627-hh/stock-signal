...
type Suggestion = { ticker: string; name: string; market: string }
...
function AddStockPanel({ onAdded }: { onAdded: () => void }) {
  ...
  const handleInputChange = (value: string) => {
    ...
    if (isKorean(value) && value.trim().length >= 1) {
      setTicker("")
      if (debounceRef.current) clearTimeout(debounceRef.current)
      setSearching(true)
      debounceRef.current = setTimeout(async () => {
        try {
          const results = await api.searchStocks(value.trim(), "KRX")
    ...
    <form onSubmit={handleAdd} className="flex items-center gap-2 flex-wrap">
      <div ref={wrapperRef} className="relative">
        <input
          ...
          placeholder="종목명(한글) 또는 티커 (예: 삼성전자, NVDA)"
          ...
          <div className="absolute top-full left-0 mt-1 w-full bg-white border border-gray-200 rounded-lg shadow-lg z-50 max-h-48 overflow-y-auto">
            {suggestions.map((s) => (
              ...
              >
                ...
                <span className="text-xs text-gray-400 font-mono ml-2">{s.ticker}</span>
              </button>
            ))}
          </div>
        )}
